...
class server:
    ...
    def __set__(self, password) -> None:
        ...
        receives.tag_config(
            "mine", foreground="#000000", background="#43a649", font=font_size
        )
        receives.tag_config(
            "others", foreground="#000000", background="#ffffff", font=font_size
        )
        receives.tag_config(
            "system", foreground="#ff0000", background="#ffff00", font=font_size
        )
        receives.tag_config(
            "message", foreground="#000000", background="#ffffff", font=font_size
        )
        ...
        while True:
            try:
                ...
                if self.check(conn):
                    ...
                    self.send_all(
                        server_conns, conn, _("[系统提示]user_append:%s") % user_name
                    )
                    threading.Thread(
                        target=self.show, args=(conn, server_conns)
                    ).start()
            except:
                break
        ...
        while True:
            try:
                ...
                self.send_all(
                    server_conns,
                    conn,
                    _("[系统提示]%s退出群聊！" % server_users[delete]),
                )
                self.send_all(
                    server_conns,
                    conn,
                    _("[系统提示]user_delete:%s" % server_users[delete]),
                )
    ...
    def save_as(self) -> None:
        file = asksaveasfilename(
            filetypes=[("LNSS " + _("聊天记录文件"), "*.lns")],
            defaultextension=".lns",
            initialfile=time.strftime("%Y%m%d%H%M%S", time.localtime(time.time())),
        )
        if file:
...
class Image_load:
    def load(
        master: Tk | None = None,
        image_file: str | bytes | None = None,
        ...
        for line in (os.popen("arp -a").read().split("\n"))[4:-1]:
            if f"192.168.{ip3}." in line:
                if "动态" in line:
                    all_ip.append(
                        f"192.168.{ip3}."
                        + line.split(f"192.168.{ip3}.")[1].split(" ")[0]
                    )
...
def smain(password_type, passwords):
    ...
    server_root.protocol(
        "WM_DELETE_WINDOW", lambda: server.destroied(main, server_root)
    )
...
class client:
    ...
    def save_as(self) -> None:
        file = asksaveasfilename(
            filetypes=[("LNSS " + _("聊天记录文件"), "*.lns")], defaultextension=".lns"
        )
        if file:
...
class Login:
    ...
    def get_sign_up_data(
        self, window: Tk | None = None, entry: list[Entry | BooleanVar] = []
    ) -> None:
        global USER
        if not window is None:
            self.swindow = window
        ...
        if self.entry[1].get():
            ...
            is_admin = False
        sql = (
            """INSERT INTO user_info (name,id,create_time,is_admin) VALUES (?,?,?,?)"""
        )
        cursor.execute(
            sql,
            (
                self.entry[0].get(),
                int(secrets.token_hex(6), 16),
...
class Demo:
    ...
    def settings(self) -> None:
        ...
        self.language_famliy = Combobox(
            language_frame, width=30, height=8, values=LANGUAGE_LIST
        )
